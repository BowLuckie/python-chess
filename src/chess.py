...
# Default settings
DEFAULT_SETTINGS = {"screen_size": 800, "evil_mode": False}  # default value
...
def get_board_mode() -> str:
    ...
    try:
        ...
        with open(path, "w") as f:
            json.dump(
                {"board_mode": "standard", "available_modes": list(BOARDS.keys())}, f
            )
        return "standard"
...
class GameState:  # this class contains all the mutable variables that migtht need to be accsed throughout the code
    ...
    def reset(self):
        ...
        self.board: Board = BOARDS.get(
            board_mode, standard_board
        )()  # .get allows us to specify a default value as apposeded to indexing which raises an error
        ...
        try:
            ...
            raise KingError(
                f"Board is missing a king! kings found: white: {self.white_king_pos if hasattr(GameState, "white_king_pos") else None}, "
...
def text_outline(
    ...
    surf = pygame.Surface(
        size if surf_size is None else (surf_size, surf_size), pygame.SRCALPHA
    )
...
exit_rect: pygame.Rect = exit_surf.get_rect(
...
def draw_board(
    screen, highlighted: coordinate | None = None, checked: coordinate | None = None
):
    """
    ...
    for row in range(8):
        for col in range(8):
            colour = COLOURS[
                (row + col) % 2
            ]  # Square color is determined by parity of (col + row)
            ...
            if (
                row,
                col,
            ) == highlighted:  # highlighted squares take priority over checked squares
        ...
        for row in range(8):
            for col in range(8):
                ...
                if piece is not None and not flipped:
                    screen.blit(
                        IMAGES[piece.image_key()],
                        (col * SQUARE_SIZE, row * SQUARE_SIZE),
                    )
                elif piece is not None and flipped:
                    screen.blit(
                        pygame.transform.rotate(IMAGES[piece.image_key()], 180),
                        (col * SQUARE_SIZE, row * SQUARE_SIZE),
                    )
    except KeyError as e:
        print(
            "\033[31mAn error has occured attempting to load some images! if you made a custom piece make sure it add it to custom[]"
        )
        print(f"{e}\033[0m")
        return
...
def draw_promotion(colour):
    ...
    for i, piece_name in enumerate(options):
        ...
        if not gamestate.white_turn:
            prom_menu_img = pygame.transform.rotate(
                prom_menu_img, 180
            )  # flip promotion options because they are their own surface
...
def draw_outcome(winner: str) -> pygame.Surface:
    ...
    text_rect = victory_text.get_rect(
        center=(BOX_WIDTH // 2, (BOX_HEIGHT // 4) - (SQUARE_SIZE * 0.5))
    )
    ...
    if not draw:
        ...
        crown_rect = crown.get_rect(midbottom=(img_rect.centerx, img_rect.top + 8))
...
def move_piece(
    ...
    gamestate.board[trow][tcol] = gamestate.board[orow][
        ocol
    ]  # dupe the piece into the new position
    gamestate.board[orow][ocol] = None  # remove old piece

    piece = gamestate.board[trow][tcol]
    if (
        piece is not None and hasattr(piece, "has_moved") and not simulate
    ):  # checks if the peice has the "has_moved" attribute
    ...
    # promotion
    if (
        (isinstance(piece, Pawn) or isinstance(piece, Soldier))
        and (trow == 7 or trow == 0)
        and not simulate
    ):
        promotion_move = True
        ...
        start_row = (
            trow if piece.colour == "w" else trow - 3
        )  # 3 squares towards the top
        for i in range(4):
            gamestate.promotion_click_locations.append((start_row + i, tcol))

    # castling
    if (
        isinstance(piece, King) and abs(tcol - ocol) == 2
    ):  # checks if distance between origin and target is 2, hence a castle has happened
        if tcol == 6:
    ...
    if (
        ...
        and target == gamestate.en_passant_square
        and gamestate.last_double_pawn is not None
    ):
        gamestate.board[gamestate.last_double_pawn[0]][
            gamestate.last_double_pawn[1]
        ] = None
    ...
    if piece is not None and not simulate:
        ...
        if (
            not ai_move and ai_active
        ):  # if the last move was human and the gamemode is set to ai
            if not (
                (isinstance(piece, Pawn) or isinstance(piece, Soldier))
                and target[0] == 0
            ):  # if human didnt move a pawn to the back rank
                if ai_can_double_move:
                    move_ai(
                        gamestate, double=True
                    )  # preform an additional move that doesnt flip the turn so the next move can flip it
...
def square_is_attacked(
    square: coordinate, looking_color: str, gamestate: GameState
) -> bool:
    """
...
def simulate_move(gamestate: GameState, origin: coordinate, target: coordinate) -> bool:
    ...
    piece = temp_state.board[origin[0]][
        origin[1]
    ]  # capture the piece before it was moved
...
def piece_clicked(gamestate: GameState, mouse_pos: coordinate) -> coordinate | None:
    ...
    if gamestate.selected_square:
        if (row, col) in gamestate.legal_moves:
            move_piece(
                gamestate, gamestate.selected_square, (row, col)
            )  # becuase legal moves is built on selection, we dont have to simulate the move
    ...
    if piece is not None:
        if (
            ai_active and piece.colour == "b"
        ):  # black pieces can never be selected under any circumstances if ai_mode is on
            return None
        if (gamestate.white_turn and piece.colour == "w") or (
            not gamestate.white_turn and piece.colour == "b"
        ):
            pseudo_moves = piece.get_legal_moves(gamestate.board, row, col, gamestate)
            gamestate.legal_moves = [
                move
                for move in pseudo_moves
                if simulate_move(gamestate, (row, col), move)
            ]  # simulate every move in psuedo moves and if they sucseed add them here
            return (row, col)
...
def event_handling():
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            ...
            if (
                not gamestate.white_turn and gamestate.game_over == False
            ):  # i forgot to add a check for is the game is over so thats why the menu has to check for squares 5,4
            ...
            if event.key == pygame.K_ESCAPE:
                import src.menu as menu  # import menu at this scope to avoid circular import
...
print(
    "\033[33mif you made a new board, add it to BOARDS and json.dump method below and delete settings.json to rebuild it. "
    "then change the board mode in the .json. board mode can only be changed if your running the .py file not the .exe\033[0m"
)
print(
    "if you are running the exe, and can see this terminal, you are running a pre-release or a debug release."
)
...
def main(ai: bool = ai_active, ai_b: bool = ai_can_double_move):
    ...
    if settings.get("evil_mode") and settings.get("board_mode", "None") in (
        "standard",
        "None",
    ):
    ...
    while running:
        ...
        square_in_check = (
            gamestate.white_king_pos
            if king_in_check(gamestate, "w")
            else gamestate.black_king_pos if king_in_check(gamestate, "b") else None
        )
        ...
        draw_board(
            screen, highlighted=gamestate.selected_square, checked=square_in_check
        )
        draw_pieces(
            screen,
            gamestate.board,
            flipped=not gamestate.white_turn and not gamestate.game_over,
        )  # becuase the whole screen will do a full rotation, we need to do a pre-emptive flip to account for it
        draw_legal_moves(screen, gamestate.legal_moves)

        if gamestate.promotion_active and gamestate.promotion_square:
            piece = gamestate.board[gamestate.promotion_square[0]][
                gamestate.promotion_square[1]
            ]
            if piece is not None:
        ...
        if (
            not gamestate.white_turn and not ai and not gamestate.game_over
        ):  # rotate the screen if its blacks turn (some elements are unaffected)
            screen.blit(rotated)
